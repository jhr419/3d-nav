#include "jie_planning/global_planner_3d.hpp"
...
GlobalPlanner3D::GlobalPlanner3D(const OctomapVoxelMap & map)
: map_(map)
{
}

GlobalPlanner3D::GlobalPlanner3D(const OctomapVoxelMap & map, Options options)
: map_(map), options_(std::move(options))
{
}

void GlobalPlanner3D::setOptions(const Options & options)
{
  options_ = options;
}
...
const std::string & GlobalPlanner3D::lastFailureReason() const
{
  return last_failure_reason_;
}

int GlobalPlanner3D::lastIterations() const
{
  return last_iterations_;
}

bool GlobalPlanner3D::isTraversable(const GridIndex & index) const
{
  return map_.isGridInBounds(index) && map_.isFree(map_.gridToWorld(index));
}

double GlobalPlanner3D::gridDistance(const GridIndex & lhs, const GridIndex & rhs) const
{
...
std::vector<GridIndex> GlobalPlanner3D::reconstructPath(
...
std::vector<Eigen::Vector3d> GlobalPlanner3D::toWorldPath(
...
bool GlobalPlanner3D::hasLineOfSight(
...
void GlobalPlanner3D::smoothPath(std::vector<Eigen::Vector3d> & path) const
{
...
void GlobalPlanner3D::resamplePath(std::vector<Eigen::Vector3d> & path) const
{
...
void GlobalPlanner3D::setFailure(const std::string & reason)
{
